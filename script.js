        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (form.hasAttribute('data-netlify')) {
                    await submitToNetlify(form, formData);
                }
                // Fallback to simulated sending
                else {
                    await simulateEmailSending(name, email, message);
                }
...
async function submitToNetlify(form, formData) {
    // Add the form name to the form data
...
async function simulateEmailSending(name, email, message) {
    return new Promise((resolve) => {
        setTimeout(() => {
            console.log('Simulated email sent:', { name, email, message });
            resolve();
        }, 2000);
    });
}
...
function showFormMessage(message, type) {
    ...
    messageDiv.style.cssText = `
        ...
        margin-top: 1rem;
        font-weight: 500;
        ${type === 'success' ? 'background: #dcfce7; color: #166534; border: 1px solid #bbf7d0;' : 'background: #fef2f2; color: #991b1b; border: 1px solid #fecaca;'}
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initNavbarScroll();
    initTypingEffect();
    initProjectLinks(); // Add this line
